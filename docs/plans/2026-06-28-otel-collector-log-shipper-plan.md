...
Environment readiness (precondition): this plan assumes a target cluster where heimdall is already deployed and running (a live `heimdall` namespace with the Loki sink, plus the `heimdall` ArgoCD Application in the `argo` namespace). Task 1 preflight and Task 6 deploy/verify all depend on that. If the target cluster predates heimdall's activation — e.g. an older k3d where the `heimdall` namespace and Application are absent, or whose ArgoCD app-of-apps is synced from a stale Gitea mirror that omits `heimdall-app.yaml` — the agent must first bring the platform up to the point where heimdall reconciles: work through the relevant nordri (cluster/GitOps bootstrap) and nidavellir (app-of-apps + heimdall-app) setup so ArgoCD deploys heimdall, and only then resume Task 1. The cluster-independent code work (Tasks 2-5, 7: the Composition step, `helm template` render-validation, kuttl tests, docs) can proceed without a live heimdall; only the live preflight (Task 1) and deploy/verify (Task 6) are gated on the environment being ready.
...
- ArgoCD Application `heimdall` in namespace `argo`; HeimdallStack claim `heimdall` in namespace `heimdall`. After sync, a new `Release` resource `heimdall-otel-collector` and a DaemonSet labelled `app.kubernetes.io/name=opentelemetry-collector` appear in the `heimdall` namespace.
...
- [ ] Wait for ArgoCD to sync the heimdall app: `kubectl -n argo wait --for=jsonpath='{.status.sync.status}'=Synced application/heimdall --timeout=300s` then confirm health: `kubectl -n argo get application heimdall -o jsonpath='{.status.health.status}{"\n"}'`. Expect `Synced` and `Healthy`. (The ArgoCD control-plane namespace is `argo` on these clusters, renamed from the upstream default `argocd` on 2026-03-21 — note this is the namespace, not the `argocd.argoproj.io/*` annotation domain, which is unchanged.) (If selfHeal/auto-sync is slow, `kubectl -n argo annotate application heimdall argocd.argoproj.io/refresh=hard --overwrite` to nudge a refresh — this is a refresh trigger, not a manual resource edit, so it does not violate the test-through-Git rule.)
