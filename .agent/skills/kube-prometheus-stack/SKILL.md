...
NOT for native AlertManager routing/templating — see sibling `alertmanager-config`. NOT for PromQL / Grafana dashboards / Loki / Tempo content — adopt `grafana/skills`.
...
`Watchdog` **always fires by design** — it's the heartbeat for dead-man's-switch integrations. Don't disable it. See sibling `alertmanager-config` → "Watchdog canary."
...
- **Disabling `Watchdog`** → kills your dead-man's-switch signal. It's supposed to always fire — wire it to a heartbeat receiver (see sibling `alertmanager-config`).
