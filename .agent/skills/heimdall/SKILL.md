...
- **Loki is not an archive — noise is dropped before it is stored.** Fallout from the 2026-09-20 Loki disk-full crashloop, where three containers were 83% of ingest. The collector drops successful (2xx/3xx) access-log lines cluster-wide, trims the k8s attribute set, and skips containers named in the claim's `logExcludeContainers`; Loki itself logs at `warn` because at `info` it was its own second-largest tenant. When a source gets loud, quiet it at the source or add it here — don't grow the PVC to fit it. The chart **replaces** lists rather than merging them, so any list under the collector `config:` must restate the preset entries it wants to keep (the self-exclusion glob is the one that bites).
- **A filling volume is tier 3, in any namespace; a nearly-full one is tier 4.** Routed by alertname (`HeimdallPVC.*|HeimdallVolume.*|KubePersistentVolume.*`), criticals by `watched: "true"`. It was tier-2 chatter until the Loki fill sat there unread for four days. The chart's own `KubePersistentVolumeFillingUp` is disabled as a duplicate of these.
